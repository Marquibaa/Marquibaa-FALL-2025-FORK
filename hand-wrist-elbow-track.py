...
# Configure MediaPipe and IKPy
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose, \
     mp_hands.Hands(static_image_mode=False, max_num_hands=1, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:

    # Load chain: ensure the path matches your URDF filename
    my_chain = ikpy.chain.Chain.from_urdf_file("actual_arm_urdf.urdf", active_links_mask=[False, True, True, True, True, True])
    print("Loaded links:", [l.name for l in my_chain.links])

    target_position = [0, 0.2, 0.1]
    target_orientation = [-1, 0, 0]

    ik = my_chain.inverse_kinematics(target_position, target_orientation, orientation_mode="Y")
    print("Initial joint angles (deg):", list(map(lambda r: math.degrees(r), ik.tolist())))
    computed_position = my_chain.forward_kinematics(ik)
    print("Computed position:", [f"{val:.2f}" for val in computed_position[:3, 3]])

    # Create controlled-size figure & axis
    fig = plt.figure(figsize=(10, 10))
    ax = fig.add_subplot(111, projection='3d')

    # Tidy window positioning — wrap in try since some backends don't expose manager.window
    try:
    ...
    plt.ion()
    plt.pause(0.001)   # let the GUI spin up
    ...
    def updatePlot():
        ...
        ax.set_ylim(-0.5, 0.5)
        ax.set_zlim(0, 0.6)
        # Use pause to process the GUI events (keeps window responsive)
        plt.pause(0.001)
    ...
    while cap.isOpened():
        ...
        # Make sure matplotlib processes GUI events every loop so it doesn't freeze
        plt.pause(0.001)
